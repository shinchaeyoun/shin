			...
			if( targetObj )
			{
				...
				else if(optionType == "append-array")
				{
					...
					if( optionArr[index][ temp ] < arr[ temp ] )
					{
						...
						//console.log(optionArr[index][ temp ] >= Number(arr[ temp ]));
						//console.log(optionArr[index][ temp ]);
						//console.log(Number(arr[ temp ]));

						if(optionArr[index][ temp ] >= Number(arr[ temp ])){
							if(target.data("option3") == "hideDropObj")
							{
								var temp = $("#" +$(this).data("hide-target") );
								temp.hide();
							}
						}
				...
				else if(optionType == "ansOn-append2")
				{
					...
					if(item.data("num") == dropItem.data("ans") )
					{
						...
						// option3 start
						if(target.data('option3') == 'ansOn-drop-new-img') {
							var newSrc = item.find("img").attr("src").split(".png")[0];
							newSrc += "_on.png";
							imgTarget.append('<img src="'+newSrc+'" alt="대체텍스트 없음"/>');
						} else {
				...
				else
				{
					...
					else
					{
						...
						{
							if(  target.data("option8") == "drop-change" )
							{
								var newSrc = item.find("img").attr("src").split(".png")[0];
								newSrc += "_on.png";
								var imgTarget = dropItem.find("img");
